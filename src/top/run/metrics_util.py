...
import top.run.box_generator as Box
...
class HitMiss(object):
    ...
    def record_hit_miss(self, metric, greater=True):
        """Records the hit or miss for the object based on the metric
        threshold."""
        # NOTE(Jiyong): greater True -> find greater than threshold / False ->
        # find worse than threshold
...
class Accuracy(object):
    ...
    def compute_accuracy(self, thresh=0.1):
        ...
        if not self._errors:
            return 0
        return len(np.where(np.array(self._errors) <= thresh)
                   [0]) * 100. / len(self._errors)
...
class IoU(object):
    ...
    def _clip_poly(self, poly, plane, normal, axis):
        """Clips the polygon with the plane using the Sutherland-Hodgman
        algorithm.
        ...
        for i, current_poly_point in enumerate(poly):
            ...
            d2 = self._classify_point_to_plane(
                current_poly_point, plane, normal, axis)
            ...
            if d2 == _POINT_BEHIND_PLANE:
                ...
                if d1 == _POINT_IN_FRONT_OF_PLANE:
                    ...
                    if not result or (not np.array_equal(
                            result[-1],
                            prev_poly_point)):
