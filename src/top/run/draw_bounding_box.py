...
Reference: https://github.com/google-research-datasets/Objectron/blob/master/objectron/dataset/graphics.py
"""
...
def draw_bbox(image, object_annotations, num_keypoints):
    """Draw annotation on the image."""

    # The object annotation is a list of 3x1 keypoints for all the annotated objects.
    # The objects can have a varying number of keypoints.
    # First we split the list according to the number of keypoints for each object.
    # This also leaves an empty array at the end of the list.
    ...
    # Convert them keypoint's `xy` value to pixel.
    keypoints = [np.multiply(keypoint, np.asarray([w, h, 1.], np.float32)).astype(int)
                 for keypoint in keypoints]

    def draw_face(object_id, face, color):
        start = keypoints[object_id][face[0], :]
        end = keypoints[object_id][face[2], :]
        cv2.line(image, (start[0], start[1]), (end[0], end[1]), color, _LINE_TICKNESS)
    ...
    for object_id in range(num_objects):
        ...
        for kp_id in range(num_keypoint):
            ...
            # If a keypoint is hidden (e.g. a subset of a larger skeleton family) do not visualize it.
            if not hidden[kp_id]:
                cv2.circle(image, (kp_pixel[0], kp_pixel[1]), _POINT_RADIUS, _COLORS[object_id % len(_COLORS)], -1)
        ...
        for edge in edges:
            if edge[0] < num_keypoint and edge[1] < num_keypoint:
                ...
                if not hidden[edge[0]] and not hidden[edge[1]]:
                    cv2.line(image, (start_kp[0], start_kp[1]), (end_kp[0], end_kp[1]), _COLORS[object_id % len(_COLORS)], _LINE_TICKNESS)
