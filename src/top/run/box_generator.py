"""
General 3D Bounding Box class & various method for the box.
...
# The vertices are ordered according to the left-hand rule,
# so the normal vector of each face will point inward the box.
FACES = np.array([[5, 6, 8, 7],     # +x on yz plane
...
UNIT_BOX = np.asarray([[0., 0., 0.],        # center
                       [-0.5, -0.5, -0.5],
                       [-0.5, -0.5, 0.5],
                       [-0.5, 0.5, -0.5],
                       [-0.5, 0.5, 0.5],
...
class Box(object):
    ...
    def apply_transformation(self, transformation):
        ...
        Group multiplication is the same as rotation concatenation.
        Therefore return new box with SE3(R * R2, T + R * T2);
        Where R2 and T2 are existing rotation and translation.
        Note we do not change the scale.
        Args:
    ...
    def fit(cls, vertices):
        ...
        Directly computes the scale of the box, then solves for orientation and translation.
        Args:
            vertices: A 9*3 array of points.
                      Points are arranged as 1 + 8 (center keypoint + 8 box vertices) matrix.
        Returns:
    ...
    def inside(self, point):
        ...
        Brings the 3D point into the local coordinate of the box.
        In the local coordinate, the looks like an axis-aligned bounding box.
        Next checks if the box contains the point.
        Args:
    ...
    def volume(self):
        ...
        For the boxes, this is equivalent to np.prod(self.scale).
        However for parallelpiped, this is more involved.
        Viewing the box as a linear function we can estimate the volume using a determinant.
        This is equivalent to sp.ConvexHull(self._vertices).volume
        Returns:
            volume (float)
        """
